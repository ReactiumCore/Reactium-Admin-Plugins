...
Capability.propagate = async (force = false) => {
    ...
    return req.then(async results => {
        ...
        if (req.propagate === true) {
            ...
            Cache.set(cacheKey, Object.values(results), Enums.cache.capability);
        }
        ...
        if (Capability.autosync === true && propagate === true && retry < 5) {
            retry += 1;
            req = API.Actinium.Cloud.run('capability-create-all', { capabilities });
            req.retry = retry;
            req.propagate = true;
            op.set(Capability.request, 'propagate', req);
        }
